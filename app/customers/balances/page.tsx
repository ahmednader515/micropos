...
export default function CustomerBalancesPage() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        setRows((j.customers || []).map((c: any) => ({ id: c.id, name: c.name, balance: Number(c.balance || 0) })))
      } finally {
        setLoading(false)
      }
    })()
  ...
  return (
    <MainLayout navbarTitle="الأرصدة الافتتاحية والمبالغ النقدية للعملاء" onBack={() => history.back()}>
