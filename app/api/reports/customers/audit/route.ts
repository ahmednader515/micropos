...
export async function GET() {
  ...
  drawRTL('فحص أرصدة العملاء', rightEdge, y, 14)
  y -= 24

  const colNameRight = rightEdge
  const colBalanceRight = rightEdge - 240
  drawRTL('العميل', colNameRight, y, 10)
  drawRTL('الرصيد الحالي', colBalanceRight, y, 10)
  y -= 16

  for (const c of customers) {
    const balance = Number(c.balance || 0)
    const nameLine = `${c.name}${c.customerNumber ? ` - رقم: ${c.customerNumber}` : ''}${c.phone ? ` - هاتف: ${c.phone}` : ''}`
    drawRTL(nameLine, colNameRight, y, 9)
    drawRTL(balance.toFixed(2), colBalanceRight, y, 9)
    y -= 14
    if (y < 40) {
